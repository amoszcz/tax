...
export const useEtoroData = () => {
    ...
    const loadFile = (fileBuffer: Buffer) => {
        try {
            ...
            const closedPositions = readExcelFile<ClosedPosition>(fileBuffer, 'Closed Positions');
            setClosedPositions(closedPositions.map(position => ({
                ...position,
                "Units / Contracts": position["Units"] ?? position["Units / Contracts"]
            })));
            setDividends(readExcelFile<Dividend>(fileBuffer, 'Dividends'));
            const activity = readExcelFile<AccountActivity>(fileBuffer, 'Account Activity');
