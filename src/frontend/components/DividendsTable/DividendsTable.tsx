...
function toDividendForTable(
  dividend: Dividend,
  exchangeRates: ExchangeRate[],
): DividendForTable {
  const exchangeRate =
    exchangeRates.find((p) => p.positionId === dividend["Position ID"]) ?? null;
  return {
    ...dividend,
    NbpExchangeRate: exchangeRate?.midExchangeRate ?? null,
    NbpExchangeRateDate: exchangeRate?.midExchangeDate ?? null,
    "Net Dividend Received (PLN)": exchangeRate?.midExchangeRate
      ? dividend["Net Dividend Received (USD)"] * exchangeRate.midExchangeRate
      : 0,
    "Withholding Tax Amount (PLN)": exchangeRate?.midExchangeRate
      ? dividend["Withholding Tax Amount (USD)"] * exchangeRate.midExchangeRate
      : 0,
  };
}
...
export const DividendsTable = (props: { dividends: Array<Dividend> }) => {
  ...
  const [dividendForTable, setDividendForTable] = useState<DividendForTable[]>(
    dividends.map((dividend) => toDividendForTable(dividend, exchangeRates)),
  );
  useEffect(() => {
    setDividendForTable(
      dividends.map((p) => toDividendForTable(p, exchangeRates)),
    );
  ...
  function handleGetExchangeRates() {
    dividends.forEach(async (dividend) => {
      ...
      getNbpExchangeRate(dayBefore).then((rate) =>
        setExchangeRates((prevState) => {
          return [
            ...prevState.filter(
              (p) => p.positionId !== dividend["Position ID"],
            ),
            {
