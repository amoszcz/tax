...
const toClosedPositionTableElement = (
  position: ClosedPosition,
  exchangeRates: ExchangeRate[],
): ClosedPositionTableElement => {
  const closeExchangeRate =
    exchangeRates.find((p) => p.positionId === position["Position ID"]) ?? null;
  const openExchangeRate =
    exchangeRates.find((p) => p.positionId === position["Position ID"]) ?? null;
  const valueOpen =
    position["Open Rate"] * parseFloat(position["Units / Contracts"]);
  const valueClose =
    position["Close Rate"] * parseFloat(position["Units / Contracts"]);
  const valueOpenPLN = openExchangeRate?.openMidExchangeRate
    ? valueOpen * openExchangeRate.openMidExchangeRate
    : null;
  const valueClosePLN = closeExchangeRate?.closeMidExchangeRate
    ? valueClose * closeExchangeRate.closeMidExchangeRate
    : null;
  return {
    ...position,
    ValueOpen: valueOpen,
    ...
    "Profit (PLN)":
      !!valueClosePLN && !!valueOpenPLN ? valueClosePLN - valueOpenPLN : null,
  };
};

export const ClosedPositionsTable = (props: {
  positions: Array<ClosedPosition>;
}) => {
  const { positions } = props;
  // const positionsTableElements = positions.map(toClosedPositionTableElement);
  const [nbpExchangeRates, setNbpExchangeRates] = useState<ExchangeRate[]>(
    positions.map((p) => ({
  ...
  const [closedPositionsTableElements, setClosedPositionsTableElements] =
    useState<ClosedPositionTableElement[]>(
      positions.map((p) => toClosedPositionTableElement(p, nbpExchangeRates)),
    );
  useEffect(() => {
    setClosedPositionsTableElements(
      positions.map((p) => toClosedPositionTableElement(p, nbpExchangeRates)),
    );
  ...
  function handleGetExchangeRates() {
    positions.forEach((position) => {
      ...
      getNbpExchangeRate(closeDate).then((rate) =>
        setNbpExchangeRates((prevState) => {
          const existingRate = prevState.find(
            (p) => p.positionId === position["Position ID"],
          );
          return [
            ...prevState.filter(
              (p) => p.positionId !== position["Position ID"],
      ...
      getNbpExchangeRate(openDate).then((rate) =>
        setNbpExchangeRates((prevState) => {
          const existingRate = prevState.find(
            (p) => p.positionId === position["Position ID"],
          );
